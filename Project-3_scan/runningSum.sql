-- ===========================================================================
-- runningSum.sql: pipelined sum by partition
...
drop function if exists runningSum_state(int, intRec) cascade;
create function runningSum_state(int, intRec) returns int language plpgsql as $f$
declare i alias for $1;
declare a alias for $2;
declare j int;
begin if a.restart
or i is null then j := a.number;
elsif a.number is null then j := i;
else j := a.number + i;
end if;
return j;
end $f$;
...
drop function if exists runningSum_final(int) cascade;
create function runningSum_final(int) returns intRec language sql as $f$
...
drop aggregate if exists runningSum(intRec) cascade;
...
with -- look at the neighbour tuple to the left to fetch its grp value
